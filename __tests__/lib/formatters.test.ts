...
describe("formatPhone", () => {
  it("formats international +49 number", () => {
    const result = formatPhone("+4964418056544");
    expect(result).toContain("+49");
    expect(result).toContain("6441");
  });

  it("formats local 0-prefixed number", () => {
    const result = formatPhone("06441-8056544");
...
describe("formatPhoneHref", () => {
  it("converts 0-prefix to +49 tel: link", () => {
    expect(formatPhoneHref("06441-8056544")).toBe("tel:+4964418056544");
  });

  it("keeps +49 prefix as tel: link", () => {
    expect(formatPhoneHref("+4964418056544")).toBe("tel:+4964418056544");
  });
});
